...
2. Install `flow-wiki` while `wiki` is still installed. Until the repository rename and published install instructions are verified, use `specify extension add --dev /path/to/spec-kit-flow-wiki` with a local checkout. After publication, use the verified installation instructions in the project README.
...
6. Verify the new extension registry, all five generated skill IDs, the effective configuration values, and exactly one active Flow Wiki ingest hook per configured event. Compare `wiki/` with the recorded baseline. Do not continue if the new config is invalid, the hook set is duplicated, or any wiki file changed unexpectedly.
7. Remove the old registration with `specify extension remove wiki --keep-config --force`. This unregisters its commands and hooks while preserving the old config directory for rollback. After every check passes, move the old config copy to the chosen rollback location and remove the inactive `.specify/extensions/wiki/` directory. Confirm the registry contains `flow-wiki` and no `wiki` entry, and that no `speckit.wiki.*` command or `speckit-wiki-*` skill remains active.
